...
CONSTANT = {"url": "https://www.lcsd.gov.hk/datagovhk/facility/facility-hssp7.json"}
...
def reformat_raw_data(ele):
    ...
    # def reformat_raw_data(totalProcess, counter, ele):
    global counter
    # totalProcess = get_list_length(data)

    # print("reformat", totalProcess, counter)
    # ele = list[1]
    # totalProcess = list[0]
    ...
    newPitchesData = {
        "UUID": ele["GIHS"],
        "hashValue": generate_hash_value(
            ele["Name_cn"], ele["Opening_hours_cn"], ele["Remarks_cn"]
        ),
        "location": {
            "name": {"en": ele["Name_en"].strip(), "cn": ele["Name_cn"].strip()},
            "address": {
            ...
            "geocode": {
                "WGS84": {
                    ...
                    "DMS": {"lat": ele["Latitude"], "lng": ele["Longitude"]},
                }
            },
            "court_no": find_court_no(ele["Court_no_en"]),
        },
        "phone": clean_phone_num(ele["Phone"]),
        "opening_hours": find_opening_hours(
        ...
        "facilities": {
            ...
            "accessibility": {
                ...
                "tactileGuidePath": determine_overall(
                    cleanFacilityString, "tactile guide path"
                ),
                "brailleDirectoryMap": determine_overall(
                    cleanFacilityString, "braille directory"
                ),
            },
    ...
    # totalcount = 0
    print(
        "Processing {} of {}, {:.1%} finished".format(
            counter, totalProcess, counter / int(totalProcess)
        ),
        end="\r",
        flush=True,
    )
...
def main(data):
    ...
    # for concurrent
    array = [(ele, totalProcess, counter) for ele in data], totalProcess
    args = ((totalProcess, counter, d) for d in data)
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        # with concurrent.futures.ProcessPoolExecutor(max_workers=7) as executor:
        # formattedObject = executor.map(lambda p: reformat_raw_data(*p), args)
        ...
        currentTime = datetime.now()
        x = open(
            "./data/cached/soccer_pitches_cleaned"
            + "_"
        ...
        x.write(
            json.dumps(newPitchesList, indent=4, sort_keys=True, ensure_ascii=False)
        )
        x.close()
